...
int my_comparer(const void *this, const void *that)
{
	...
	Process* pointer1 = (Process*)this;
	Process* pointer2 = (Process*)that;

	if ( pointer1->priority < pointer2->priority ){
		return 1;	
	}
	
	else if ( pointer1->priority > pointer2->priority ){
		return -1;
	}

	//check arrival time comparisons since priority is the same
	if ( pointer1->arrival_time < pointer2->arrival_time ){
		return -1;
	}
	
	else if ( pointer1->arrival_time > pointer2->arrival_time ){
		return 1;
	}
