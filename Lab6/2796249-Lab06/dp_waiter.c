...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    //vars for availability
    while(!(left_chop_available(me) && right_chop_available(me)))
    {
    	pthread_cond_wait(&(me->can_eat), &waiter);
    }
    
    int* leftAvailability = left_chop_available(me);
    int* rightAvailability = right_chop_available(me);
    
    //make neither available
    *leftAvailability = 0;
    *rightAvailability = 0;
    ...
    *rightAvailability = 1; 
    ...
    pthread_mutex_unlock(&waiter);//unlock 
